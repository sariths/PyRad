...
HPAT = re.compile('^(VIEW|EXPOSURE|PIXASPECT|PRIMARIES|COLORCORR)=|[^ ]*(rpict|rview|pinterp) ')
...
class Pveil(ProcMixin):
	...
	def run(self):
		...
		p = self.call_one(fg_cmd, 'extract glare values', out=PIPE)
		if self.donothing:
			fg_data = None
		else:
		...
		# we need to preserve the original exposure values and some other stuff
		with open(self.imgfile, 'r') as inf:
			line = inf.readline().strip()
			while line:
				if HPAT.search(line): print(line)
				line = inf.readline().strip()
		sys.stdout.flush()
	...
	def write_calfile(self, cal_fd, gv_table):
		...
		for k,vals in zip((b'SDx',b'SDy',b'SDz',b'I'), sel_lists):
			if len(vals) < 2: os.write(cal_fd, k + b'(x):select(1,')
			else: os.write(cal_fd, k + b'(i):select(i,')
